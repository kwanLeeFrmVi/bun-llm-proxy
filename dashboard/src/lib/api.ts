...
export const api = {
  ...
  providers: {
    ...
    getEnabledModels: (id: string) =>
      request<{
        provider: string;
        models: string[];
      }>(`/api/providers/${id}/enabled-models`),
    addEnabledModel: (id: string, model: string) =>
      request<{
        provider: string;
        models: string[];
      }>(`/api/providers/${id}/enabled-models`, {
    ...
    removeEnabledModel: (id: string, model: string) =>
      request<{
        provider: string;
        models: string[];
      }>(`/api/providers/${id}/enabled-models`, {
    ...
    setEnabledModels: (id: string, models: string[]) =>
      request<{
        provider: string;
        models: string[];
      }>(`/api/providers/${id}/enabled-models`, {
  ...
  nodes: {
    validate: (data: unknown) =>
      request("/api/provider-nodes/validate", { method: "POST", body: JSON.stringify(data) }),
    update: (
      id: string,
      data: { name?: string; baseUrl?: string; prefix?: string; apiType?: string }
    ) =>
      request<{ node: ProviderNode }>(`/api/provider-nodes/${id}`, {
        method: "PUT",
  ...
  keys: {
    ...
    create: (name: string, userId?: string | null) =>
      request("/api/keys", {
        method: "POST",
  ...
  usage: {
    ...
    requestDetails: (params: Record<string, string> = {}) => {
      ...
      return request<{ rows: unknown[]; total: number }>(
        `/api/usage/request-details${qs ? `?${qs}` : ""}`
      );
    },
    leaderboard: (period = "24h") =>
      request<{
        leaderboard: Array<{
          userId: string;
          username: string;
  ...
  models: {
    list: () =>
      request<{
        data: { id: string; created: number; owned_by?: string; combo_models?: string[] }[];
      }>("/v1/models"),
  },
