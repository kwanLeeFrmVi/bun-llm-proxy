...
export default function MavisUsage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load from Mavis");
    } finally {
      setIsLoading(false);
    }
  }, [range]);
    ...
    <div className="flex flex-col gap-6">
      ...
      {error && (
        <div className="flex gap-3 rounded-xl bg-[var(--surface-container-lowest)] p-6 border border-[rgba(203,213,225,0.6)]">
          ...
          <div>
            <p className="text-[13px] font-600 text-[#ef4444]">Failed to load Mavis data</p>
              ...
              value={
                ...
                (usage!.models.length > 0
                  ? usage!.models
                      .reduce(
                        (sum, m) =>
                          sum + estimateCost(m.input_tokens, m.output_tokens, pricing[m.model]),
                        0
                      )
                      .toFixed(2)
                  : "0.00")
